...
export default function Home({ _jsonData, _posData }) {
  ...
  function updateLinksData() {
    ...
    setLinksData(
      _jsonData.getHeroCombinationWinLose.map((d) => {
        if (
          activeHero.indexOf(d.hero1) != -1 ||
          activeHero.indexOf(d.hero2) != -1
        ) {
          return {
            source: d.hero1,
            target: d.hero2,
      ...
      (async () => {
        const jsonData = {
          ...
          getHeroCombinationWinLose: _jsonData.getHeroCombinationWinLose.filter(
            (e) => {
              return (
                activeHero.indexOf(e.hero1) != -1 ||
                activeHero.indexOf(e.hero2) != -1
              );
            }
          ),
        };
